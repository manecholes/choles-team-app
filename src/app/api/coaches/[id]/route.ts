...
import { deleteCoach, updateCoach } from "@/server/services/staff.service";
...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const coach = await updateCoach(clubId, Number(params.id), data);
    return jsonOk({ coach });
  } catch (err) {
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const clubId = resolveClubScope(user);
    if (!clubId) throw new Error("Club no resuelto");
    await deleteCoach(clubId, Number(params.id));
